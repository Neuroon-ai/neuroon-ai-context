...
- `mcp__sonarqube__*` — el MCP **oficial de SonarSource** (`sonarsource/sonarqube-mcp`, contenedor, stdio): quality gate, issues y **security hotspots** de una PR. Existe porque Sonar era la única puerta de la CI que bloquea sin que se pueda decir qué fichero es: un hotspot no sale por `gh run view --log` y su API responde 401 sin token. `SONARQUBE_TOKEN` tiene que ser **de usuario** — el server no funciona con tokens de proyecto ni globales— y es distinto del `SONAR_TOKEN` de la CI, que es de análisis. Ojo al alcance: este MCP también puede **cambiar** el estado de un hotspot, y ese límite lo pone el permiso del usuario dueño del token, no el server.
...
- `mcp__dataforseo__*` — **dataforseo-mcp-server** (oficial, stdio local): datos SEO/SERP — keyword research, análisis de SERP, backlinks, gap de competidores. Venía del monolito (`neuroon-platform/.mcp.json`), donde las credenciales estaban **en claro dentro del JSON**; aquí entra con el patrón de la casa (`DATAFORSEO_USERNAME`/`DATAFORSEO_PASSWORD` en el `.env`, wrapper `sh -c`). Es el único MCP de esta lista **sin token de solo-lectura posible**: DataForSEO no distingue scopes, y **cada llamada gasta créditos de pago reales**. Por eso el límite no es técnico sino de criterio: no barrer keywords "por explorar", consultar lo que se va a usar.
